...
Vec3 projectOnPlane(Vec3 p){
	Vec3 n0 = (p - Proj::P0).normalize();
	double d = fabs( Proj::P_N0*(Proj::P0 - Proj::P_P0) );
	double t = d / (Proj::P_N0*n0);
	return Proj::P0 + n0*fabs(t);// + Proj::P_N0*0.1*((t<0)?1:-1);
};
...
NURBS::NURBS(Vec3 * cps, int size, int deg, double k){
	...
	for(int i=0; i<_num_controls; i++){
		_controls[i].set(projectOnPlane(_controls[i]));
...
BSpline::BSpline(Vec3 * cps, int size, double step, bool closed){
	_isclosed = closed;
	_size = size;
	double len = ArrCurve::length(cps, size);
	step = (step>len)?len-0.0000001:step;
	_pts = ArrCurve::resample(cps, size, step, _pnum);
	if (_isclosed && (_pts[_pnum-1] - _pts[0]).norm()<step*0.75)
		_pnum--;

	for(int i=0; i<_pnum; i++){
		_pts[i].set(projectOnPlane(_pts[i]));
		ControlPoint::create(this, &_pts[i]);
	}
}

const Vec3& BSpline::getP(double T){
	Vec3 * p = new Vec3(0,0,0);
	int cid = (int)(T * _pnum);
	double t = T * _pnum - cid;
	double b[3] = {((1-t)*(1-t)*0.5), (-t*t+t+0.5), (t*t*0.5)};
	for(int j=0; j < 3; j++){
		int pi = cid+j-1;
		if (!_isclosed)
			pi = (pi < 0)?0:((pi>=_pnum)?(_pnum-1):pi);
		else
			pi = (pi+_pnum)%_pnum;
		p[0] = p[0] + _pts[pi]*b[j];
	}
...
BSpline* BSpline::create(CurvePtr c, double step){
	return new BSpline(c->toArr(), c->size(), step, c->isClosed());
}
...
int ControlPoint::handleMouseEvent(int state, Vec3 p){
	...
	Vec3 pp = projectOnPlane(p);
	double rad = (*_p - pp).norm();
	if (!state || rad>(_rad/(Proj::P0-p).norm()*(Proj::P0-pp).norm() ))
		return 0;
	_selected = true;
	return 1;
}
