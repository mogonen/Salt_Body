...
__device__ void FindClosestDistanceAndPrimitiveID(const float3 &cPos, float &rClosestDist, float3 * pcFoundNearestPos, float3 * pcFoundNearestNormal) 
{
 ...
 CudaBoundingVolumeHierarchy::NodeEncoding
  ...
  while (nNode || isCurrentNodeSet)
  {
        ...
        else
        {
          ...
          { 
            *pcFoundNearestPos = cNearestPos;
            ...
            pcFoundNearestNormal->x = cB.y * cC.z - cB.z * cC.y;
            pcFoundNearestNormal->y = cB.z * cC.x - cB.x * cC.z;
            pcFoundNearestNormal->z = cB.x * cC.y - cB.y * cC.x;
    ...
    else
    {
      ...
      bool 
        isR0 = (rMinDist0 * 1.0f < rClosestDist),
        isR1 = (rMinDist1 * 1.0f < rClosestDist);
...
__global__ void FindMinimalDistance(const int nDim2Modulo, int isLine)
{
      ...
      iDim2 < cMC.m_anMax[2])
  {
    ...
    if (isLine)
    {
      FindClosestDistanceAndPrimitiveID<true>(cPos, rClosestDist,&cFoundNearestPos,&cFoundNearestNormal);
      rClosestDist = sqrtf(rClosestDist);
    }
    else
    {
      FindClosestDistanceAndPrimitiveID<false>(cPos, rClosestDist,&cFoundNearestPos,&cFoundNearestNormal);
      rClosestDist = sqrtf(rClosestDist);

      cFoundNearestNormal = dnorm(cFoundNearestNormal);
      cFoundNearestPos = dnorm(dsub(cPos,cFoundNearestPos));
      
      float
        rAngle = ddot(cFoundNearestNormal,cFoundNearestPos);

      rClosestDist *= rAngle;

      //if (rAngle < 0) rClosestDist =-rClosestDist;
