...
CurveBoss * ExternalViewContextWidget::cboss;
...
ExternalViewContextWidget::ExternalViewContextWidget(QWidget* pParent, int argc, char ** argv) : HueQtDefaultViewerWidget(pParent)
{
  ...
  ExternalViewContextWidget::cboss = CurveBoss::getBoss();
  ...
  Hue::ProxyLib::ConfigRendering::Instance()->SetVolumeSliceLoadTimeMax(2000);
 
  Hue::ProxyLib::ConfigMemoryManagement::Instance()->SetGPUCacheMax(500);
  Hue::ProxyLib::ConfigMemoryManagement::Instance()->SetGPUDecompressMax(64);

  _pLineSet = pProject->Shapes().CreateDirectShape();
...
*/
  ...
  Hue::ProxyLib::ShapeInstanceViewContext *
	  pLineSetVC = pViewContextContainer3->ViewContexts().CreateShapeInstanceViewContext(); // This object instantiates a Shape instance object

  Hue::ProxyLib::ShapeInstance *
	  pLineSetSI = pProject->MovableObjects().CreateShapeInstance(); // This object instantiates a Shape instance object

  pLineSetVC->SetShapeInstance(pLineSetSI);
  pLineSetSI->SetShape(_pLineSet);

  //TorusShape *pTorusShape = pProject->Shapes().CreateTorusShape();
  //pLineSetSI->SetShape(pTorusShape);
  ...
  Hue::ProxyLib::ExternalViewContext *
    pExternalViewContext = pViewContextContainer3->ViewContexts().CreateExternalViewContext(); // This is an view context which provides a hook (through custom events) for performing direct OpenGL rendering from outside the component
  ...
  pMarkerShape->SetMemoryCachePolicy(Hue::ProxyLib::CachePolicy::NoCaching/*The produced shape contents are not kept in memory*/); // This parameter controls the policy used for caching produced data in memory (Default: KeepLastInMemoryWhileValid)
  pMarkerShape->SetStyle(Hue::ProxyLib::MarkerShapeStyle::Axis/*Axis*/); // This property defines what kind of arrow to create (Default: RoundArrowFromCenter)
  ...
  pMarkerShape2->SetMemoryCachePolicy(Hue::ProxyLib::CachePolicy::NoCaching/*The produced shape contents are not kept in memory*/); //  (Default: KeepLastInMemoryWhileValid)
  pMarkerShape2->SetStyle(Hue::ProxyLib::MarkerShapeStyle::Sun/*Sun*/); //  (Default: RoundArrowFromCenter)
  ...
  pMarkerShape3->SetMemoryCachePolicy(Hue::ProxyLib::CachePolicy::NoCaching/*The produced shape contents are not kept in memory*/); //  (Default: KeepLastInMemoryWhileValid)
  pMarkerShape3->SetStyle(Hue::ProxyLib::MarkerShapeStyle::FlatArrowFromCenter/*Flat arrow (from center)*/); //  (Default: RoundArrowFromCenter)
  ...
  pOrbitCameraController->SetPivotPointMode(Hue::ProxyLib::PivotPointMode::Centered/*Pivot point is kept in scene center*/); // This property maintains pivot point status. (Default: Floating)
  ...
  pExternalViewContext->SetGlobalBoundingBox(BoundingBox(DoubleVector3(-999,-999,-999),DoubleVector3(999,999,999)));
  InitSphere(1.0f);
  //pExternalViewContext->SetGlobalBoundingBox(BoundingBox(DoubleVector3(-1.0, -1.0, -1.0), DoubleVector3(1.0, 1.0, 1.0)));
 Hue::ProxyLib::VDS *
	  restoreVDS = pProject->RestoreVDSFromFileName("c://data//subsa");

//	  restoreVDS = pProject->RestoreVDSFromFileName("c://data//2006_RP_PSTM");
  ...
  Hue::ProxyLib::VolumeBox
	  ...
	  *distVB = volumeBox->Attached3DObj().CreateVolumeBox();
...
Hue::ProxyLib::SetFrontfaceStateNode *
  ...
  Hue::ProxyLib::RSTViewContext *
    pRSTViewContext = pViewer->ViewContextContainerInsideAndOutsideMagnifier()->ViewContexts().CreateRSTViewContext(); // This object instantiates the contents of a Render state tree in a 3D render layer

  pRSTViewContext->SetRST(pRenderStateTree);


 // Create a vds for the distvolume
 Hue::ProxyLib::VDS
	 * noiseVDS = pProject->VDSs().CreateVDSGenNoise();

 distVB->SetValueVDS(noiseVDS);

 // Create TF, VolumeRS
 Hue::ProxyLib::TransferFunction *
    pDistTF = pProject->TransferFunctions().Create(); // A transfer function is an object which contains information about how to map from raw, original data values to final colors and transparency. They can be applied to volume boxes and mesh surfaces using VM programs.

 Hue::ProxyLib::VolumeRS *
    pDistRS = pProject->VolumeRenderStates().Create(); // This is a volume render state which defines how volume rendering should be performed.

  pDistRS->SetVMVolumeProgram(vmVolumePrograms1D1SReadout[0]); // This property defines a reference to the volume VM program to be used by this particular render state
  pDistRS->SetVolumeBox0(distVB); // This property defines a reference to a volume dataset used by a VM program
  pDistRS->SetTransferFunction0(pDistTF); // This property defines a reference to a 1D transfer function used by a VM program

  Hue::ProxyLib::SetVolumeStateNode
	*pDistVolumeState = pSetFrontfaceStateNode2->ChildNode().CreateSetVolumeStateNode();

 // pDistVolumeState->SetVolumeStateSet(pDistRS);

  // Create a distance plugin
    // Create distance VDS
    Hue::ProxyLib::VDSFilter
      * pVDSDistance = pProject->VDSs().CreateVDSFilter();

    pVDSDistance->SetLODLevels(LODLevels::LODLevelNone);
    pVDSDistance->SetVDSFilterPlugin(RootObject::Instance()->Plugins().FindByName("3DBlur"));
/*
    ...
    paramSizeDim0->SetValue(200);
    paramSizeDim1->SetValue(100);
    paramSizeDim2->SetValue(100);

	Hue::ProxyLib::TorusShape
		*pTorusShape = pProject->Shapes().CreateTorusShape();

    paramShape->SetShape(pTorusShape);
*/
	//distVB->SetValueVDS(pVDSDistance);
...
ExternalViewContext_RenderOpaqueExternalHandler(Hue::ProxyLib::RenderOpaqueExternalEventContext *context, Hue::ProxyLib::int64 subscriptionID)
{
  ...
  ExternalViewContextWidget::cboss->drawGL();
...
{   if (sketching_on){
		//strokeman->HandleMouseMotion(toWorldOnPlane(pMouseEvent->x(),pMouseEvent->y(), p0, n0));
		strokeman->HandleMouseMotion(toWorld(pMouseEvent->x(), pMouseEvent->y()));
		float ai = pViewer->RenderLayer3DOutsideMagnifier()->AmbientIntensity();
		pViewer->RenderLayer3DOutsideMagnifier()->SetAmbientIntensity(ai+0.001 *(1- 2*(pMouseEvent->x()%2)) );
	}
...
void ExternalViewContextWidget::mousePressEvent(QMouseEvent * pMouseEvent){
	...
	if (sketching_on){
		...
		Trace2DEventContext *
			pcEventContext = this->Perform2DTrace(QPoint(pMouseEvent->x(), pMouseEvent->y()));

		if (!pcEventContext->ObjectHit())
			return;
	 
		Hue::Util::DoubleVector3 dp0 = pcEventContext->GlobalPosition();
		Hue::Util::FloatVector3  fn0 = pcEventContext->GlobalNormal();
		//p0.set(dp0.X, dp0.Y, dp0.Z); 
		//n0.set(fn0.X, fn0.Y, fn0.Z);
		Hue::Util::DoubleVector3  cam = pViewer->OwnedCamera()->Position();
		Proj::P0.set(cam.X, cam.Y, cam.Z);
		Proj::P_P0.set(dp0.X, dp0.Y, dp0.Z);
		Proj::P_N0.set(fn0.X, fn0.Y, fn0.Z); 
...
void ExternalViewContextWidget::keyReleaseEvent(QKeyEvent * pQKeyEvent){
	...
	if (pQKeyEvent->text()=="z" && sketching_on){
		strokeman->deleteLast();
	}

	if (pQKeyEvent->text()=="l" && sketching_on){
		//cboss->step();
		createLineSet();
	}
...
void ExternalViewContextWidget::createLineSet(){
	 
	_pLineSet->EnterEditMode();
	_pLineSet->Vertices().Clear();
	_pLineSet->Lines().Clear();
	
	list<StrokePtr>* clist = ExternalViewContextWidget::strokeman->getStrokes();
	for(list<StrokePtr>::iterator it = clist->begin(); it!=clist->end(); it++){
		StrokePtr s = (*it);
		if (!s || s->size()<3 || !s->curve())
			continue;
		int iV0 =-1;
		for(double t = 0; t<=1; t+=0.01){
			Vec3 p = s->curve()->getP(t);
 			_pLineSet->Vertices().Add(DoubleVector3(p.x, p.y, p.z));
			 int iV1 =_pLineSet->Vertices().Count() - 1;
			 if (iV0!=-1)
				_pLineSet->Lines().Add(Line(iV0, iV1));
			 iV0 = iV1;
		}
	}

//    _pLineSet->Vertices().Add(DoubleVector3(-1,-1,-1));
//	_pLineSet->Vertices().Add(DoubleVector3(1,1,1));
//	_pLineSet->Lines().Add(Line(0,1));

   _pLineSet->ExitEditMode();
